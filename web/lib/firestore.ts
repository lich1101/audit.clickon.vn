...
export function mapAuditRun(docId: string, data: DocumentData): AuditRun {
  return {
    ...
    checklistText: data.checklistText ?? null,
    aiProvider: ["openai", "deepseek", "gemini", "gemini_deep_research"].includes(data.aiProvider) ? data.aiProvider : "openai",
    aiModel: data.aiModel ?? null,
    step2AiProvider: ["openai", "deepseek", "gemini", "gemini_deep_research"].includes(data.step2AiProvider) ? data.step2AiProvider : data.aiProvider ?? "openai",
    step2AiModel: data.step2AiModel ?? data.aiModel ?? null,
    step3AiProvider: ["openai", "deepseek", "gemini", "gemini_deep_research"].includes(data.step3AiProvider) ? data.step3AiProvider : data.aiProvider ?? "openai",
    step3AiModel: data.step3AiModel ?? data.aiModel ?? null,
    deepResearchResearchProvider: ["perplexity", "gemini_deep_research"].includes(data.deepResearchResearchProvider) ? data.deepResearchResearchProvider : "perplexity",
    deepResearchResearchModel: data.deepResearchResearchModel ?? "sonar-deep-research",
    deepResearchReasoningProvider: ["openai", "gemini"].includes(data.deepResearchReasoningProvider) ? data.deepResearchReasoningProvider : "openai",
    deepResearchReasoningModel: data.deepResearchReasoningModel ?? "gpt-5.5",
    deepResearchFormatterProvider: ["openai", "deepseek", "gemini"].includes(data.deepResearchFormatterProvider) ? data.deepResearchFormatterProvider : "openai",
