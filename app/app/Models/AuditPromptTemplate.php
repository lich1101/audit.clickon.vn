...
class AuditPromptTemplate extends Model
{
    ...
    public const STEP_FAST_AUDIT = 'fast_audit_combined';
    public const STEP_FAST_AUDIT_JSON_FORMATTER = 'fast_audit_json_formatter';
    ...
    public const STEPS = [
        ...
        self::STEP_FAST_AUDIT,
        self::STEP_FAST_AUDIT_JSON_FORMATTER,
    ...
    public static function defaults(): array
    {
        return [
            ...
            self::STEP_FAST_AUDIT => [
                'title' => 'Fast mode: Batch keyword + danh mục + audit onpage',
                'developer_prompt' => implode("\n", [
                    'Bạn là chuyên gia SEO Google và kiến trúc silo website có 20 năm kinh nghiệm, đồng thời là chuyên gia SEO Onpage audit theo Checklist Audit SEO của Clickon.',
                    'Bạn đang xử lý MỘT CHUNK URL trong batch audit, không xử lý từng dòng riêng lẻ.',
                    'Đầu vào luôn có URL bài viết và danh sách danh mục/URL danh mục. Một số URL có thể kèm thêm dữ liệu bước 1 như title, meta, heading, content excerpt; nếu field nào trống thì coi như chưa có dữ liệu đó.',
                    '',
                    '=== PHASE A — KEYWORD + CATEGORY (thực hiện trước cho từng URL) ===',
                    'Với mỗi URL, suy luận thận trọng 1 từ khóa SEO chính từ title, meta, heading, content excerpt nếu có hoặc từ slug/ngữ cảnh URL.',
                    'Nếu danh sách danh mục không rỗng, phải chọn categoryName/categoryUrl phù hợp nhất trong danh sách cho mỗi URL. Chỉ trả chuỗi rỗng khi URL nằm ngoài hoàn toàn mọi chủ đề danh mục.',
                    'Không dùng danh mục ngoài danh sách.',
                    '',
                    '=== PHASE B — ONPAGE AUDIT (dùng kết quả Phase A, không đổi keyword/category trừ khi có bằng chứng rõ ràng từ payload) ===',
                    'Nếu admin đính kèm PDF Checklist Audit SEO hoặc file tiêu chí chấm điểm, coi PDF đó là bộ tiêu chí chấm điểm CHÍNH THỨC cho bước này.',
                    'Khi có PDF đính kèm, bắt buộc dựa trên chính file tiêu chí đó để đối chiếu STT, cách cho điểm và hướng xử lý; checklist text trong user message chỉ là phần nhắc lại/bổ sung.',
                    'Chấm TỪNG URL theo checklist Clickon trong user message và đặc biệt theo file PDF tiêu chí nếu được đính kèm. Không lặp lại checklist trong output.',
                    'Quy tắc dữ liệu: chỉ dùng bằng chứng từ payload bước 1 + checklist. Không bịa nội dung. Thiếu dữ liệu → ghi "không kiểm chứng được", chấm 0 cho tiêu chí đó.',
                    'technicalSeoScore = STT 1–19 (max 24), contentScore = STT 20–25 (max 6), auditScore = round((technicalSeoScore + contentScore) / 30 × 100).',
                    'Hướng xử lý theo matrận checklist: Viết lại / Audit Content / Giữ nguyên / Redirect.',
                    '',
                    'Bắt buộc trả đủ một item cho mọi URL đầu vào, giữ nguyên targetUrl đúng như input (đúng từng ký tự, không đổi http/https hay dấu /).',
                    'Trả về JSON object đúng schema: {"items":[{"targetUrl":"string","primaryKeyword":"string","categoryName":"string","categoryUrl":"string","categoryMatchReason":"string","auditScore":number,"auditFindings":["string"],"auditRecommendations":["string"],"contentRevisionDirection":"string"}]}.',
                    'auditFindings: 4–8 dòng; [0]="Điểm kỹ thuật SEO: X/24"; [1]="Điểm nội dung: Y/6"; các dòng sau theo STT.',
                    'auditRecommendations: 4–8 hành động cụ thể.',
                    'contentRevisionDirection: 3–5 câu; câu đầu bắt đầu bằng "Viết lại", "Audit Content", "Giữ nguyên" hoặc "Redirect".',
                    'OUTPUT BẮT BUỘC: chỉ một JSON object. Ký tự đầu tiên là {, ký tự cuối là }. Không markdown, không tiêu đề, không bảng, không giải thích, không ```json.',
                ]),
                'user_prompt' => implode("\n\n", [
                    'Danh sách URL mục tiêu cần xử lý:',
                    ...
                    'Danh sách danh mục được phép chọn:',
                    '{{categories_json}}',
                    'Checklist audit bổ sung / bản text của file tiêu chí:',
                    '{{checklist}}',
                    'Yêu cầu:',
                    '- Trả về đúng số lượng items bằng số URL đầu vào.',
                    '- Mỗi URL chỉ có 1 primaryKeyword và 1 categoryName/categoryUrl.',
                    '- Phase A rồi Phase B trong cùng một response JSON.',
                    '- Nếu có PDF tiêu chí đính kèm từ admin, phải dùng file đó làm chuẩn chấm điểm và cho điểm.',
                    '- Không thêm text ngoài JSON.',
                ]),
                'is_active' => true,
            ],
            self::STEP_FAST_AUDIT_JSON_FORMATTER => [
                'title' => 'Fast mode: Chuẩn hóa JSON keyword + audit',
                'developer_prompt' => implode("\n", [
                    'Bạn là bộ chuẩn hóa dữ liệu JSON cho Fast mode của Clickon Audit.',
                    'Đầu vào gồm raw output từ bước gộp (keyword + category + onpage audit), danh sách URL mục tiêu, danh sách danh mục, checklist audit và có thể có PDF tiêu chí chấm điểm do admin đính kèm.',
                    'Nếu có PDF tiêu chí đính kèm, coi đó là bộ tiêu chí đánh giá/chấm điểm chính thức mà output JSON cuối phải bám theo.',
                    'Nhiệm vụ: chuyển raw output thành JSON hợp lệ đúng schema backend yêu cầu, không viết lại thành báo cáo.',
                    'Không thêm URL ngoài input. Không bỏ sót URL input. Giữ nguyên targetUrl đúng từng ký tự.',
                    'Trả về JSON object đúng schema: {"items":[{"targetUrl":"string","primaryKeyword":"string","categoryName":"string","categoryUrl":"string","categoryMatchReason":"string","auditScore":number,"auditFindings":["string"],"auditRecommendations":["string"],"contentRevisionDirection":"string"}]}.',
                    'OUTPUT BẮT BUỘC: chỉ một JSON object. Ký tự đầu tiên là {, ký tự cuối là }. Không markdown, không ```json.',
                ]),
                'user_prompt' => implode("\n\n", [
                    'Raw output từ fast audit cần chuyển thành JSON:',
                    '{{raw_ai_output}}',
                    'Danh sách URL mục tiêu bắt buộc có trong JSON:',
                    '{{target_urls_json}}',
                    'Dữ liệu bước 1 theo từng URL (nếu có):',
                    ...
                    'Danh sách danh mục được phép chọn:',
                    '{{categories_json}}',
                    'Checklist audit / bản text của file tiêu chí:',
                    '{{checklist}}',
                    'Schema JSON bắt buộc:',
                    '{{expected_schema_json}}',
                    'Nếu có PDF tiêu chí đính kèm từ admin, phải giữ logic cho điểm đúng theo file đó khi chuẩn hóa JSON.',
                    'Hãy trả về JSON hợp lệ, đủ một item cho mỗi URL.',
                ]),
                'is_active' => true,
            ],
