...
- **Proxy (chỉ admin):** bật/tắt, nguồn GitHub HTTP/SOCKS5, proxy thủ công, số proxy mỗi Run — cấu hình tại **Admin → Keyword Rank Settings**. User bấm Run → server trả danh sách proxy theo cấu hình admin; extension xoay IP qua `chrome.proxy` **trước mỗi keyword**. User **không** nhập hay lưu proxy.
