...
class FirebaseIdentityService
{
    ...
    {
        try {
            if (LocalAuthToken::enabled() && str_starts_with($idToken, 'local.')) {
                $claims = LocalAuthToken::verify($idToken);
                $uid = $claims['uid'];
                $email = $claims['email'];
                $displayName = $claims['name'];
                $actor = $this->creditService->ensureUser($uid, $email, $displayName !== '' && $displayName !== null ? $displayName : null);
